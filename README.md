# Portofolio — Restu Nurrokhman (v3.2)
...
## Yang berubah di v3.2

- **Intro dihapus total**: overlay 96-frame logo + audio (`intro.mp3`)
  sudah tidak ada. Situs langsung menampilkan hero begitu halaman
  dimuat (bagian hero tetap muncul dengan animasi *reveal* biasa lewat
  `IntersectionObserver`, sama seperti section lain). Folder
  `assets/frames/` dan `assets/audio/` ikut dihapus (ukuran project
  turun drastis).
- **Logo baru**: logo `RN` (`assets/img/logo-web.png`) dipasang di
  navbar sebagai logo utama situs (menggantikan ikon `</>`) sekaligus
  jadi favicon tab browser. Logo kedua (`assets/img/logo-hamburger.png`)
  dipasang di bagian atas panel menu hamburger.
